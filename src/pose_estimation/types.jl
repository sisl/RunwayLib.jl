...
struct OptimizationConfig{CT, ST, GT}
    ...
    function OptimizationConfig(
            max_iterations::Int, convergence_tolerance::CT,
            step_tolerance::ST, gradient_tolerance::GT
        ) where {CT, ST, GT}
        if max_iterations <= 0
            throw(ArgumentError("max_iterations must be positive"))
        end

        return new{CT, ST, GT}(max_iterations, convergence_tolerance, step_tolerance, gradient_tolerance)
    end
end
...
const DEFAULT_OPTIMIZATION_CONFIG = OptimizationConfig(
    1000,           # max_iterations
    1.0e-6,         # convergence_tolerance
    1.0e-8,          # step_tolerance
    1.0e-6           # gradient_tolerance
)
