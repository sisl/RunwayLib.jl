...
@testset "Pose Estimation" begin
        ...
        @testset "Optimization Setup" begin
                ...
                observed_corners = SA[ProjectionPoint(100.0 * 1pixel, 0.0 * 1pixel), ProjectionPoint(200.0 * 1pixel, 0.0 * 1pixel)]
                ...
                @test total_error >= 0 * (1pixel)^2
                @test isfinite(ustrip(total_error))

                # Test typical reprojection error magnitudes
                @test all(error_norms .< 100 * 1pixel)  # Should be reasonable
                typical_good_error = 0.5 * 1pixel
                typical_poor_error = 5.0 * 1pixel
                @test typical_good_error < 1.0 * 1pixel
                @test typical_poor_error > 2.0 * 1pixel
        end
        ...
        @testset "Convergence Criteria" begin
                ...
                residual_history = SA[10.0*1pixel, 5.0*1pixel, 2.0*1pixel, 1.0*1pixel, 0.5*1pixel, 0.49*1pixel, 0.48*1pixel]
                tolerance = 0.5 * 1pixel
                ...
                excellent_residual = 0.1 * 1pixel    # Sub-pixel accuracy
                good_residual = 0.5 * 1pixel         # Good accuracy
                poor_residual = 2.0 * 1pixel         # Poor accuracy

                @test excellent_residual < good_residual < poor_residual
                @test excellent_residual < 0.2 * 1pixel
                @test good_residual < 1.0 * 1pixel
                @test poor_residual > 1.0 * 1pixel
        end
end
