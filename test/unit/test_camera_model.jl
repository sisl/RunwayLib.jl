...
@testset "Camera Model" begin
        @testset "Camera Parameters" begin
                ...
                optical_center_u = 2047.5 * 1pixel  # Principal point x
                optical_center_v = 1499.5 * 1pixel  # Principal point y

                @test focal_length > 0.0u"mm"
                @test pixel_size > 0.0u"μm"
                @test image_width > 0 * 1pixel
                @test image_height > 0 * 1pixel
                @test optical_center_u ≈ image_width / 2 - 0.5 * 1pixel
                @test optical_center_v ≈ image_height / 2 - 0.5 * 1pixel
        ...
        @testset "Projection Functions" begin
                ...
                @test uconvert(1pixel, focal_length_px) ≈ 7246.377 * 1pixel rtol = 1e-3
        end
