...
@testset "Coordinate Systems" begin
    ...
    @testset "ProjectionPoint" begin
        ...
        pp = ProjectionPoint(100.0u"pixel", 200.0u"pixel")
        @test pp.x == 100.0u"pixel"
        @test pp.y == 200.0u"pixel"
        
        # Test FieldVector interface
        @test length(pp) == 2
        @test pp[1] == 100.0u"pixel"
        @test pp[2] == 200.0u"pixel"
        
        # Test arithmetic operations
        pp2 = ProjectionPoint(50.0u"pixel", 75.0u"pixel")
        pp_sum = pp + pp2
        @test pp_sum == ProjectionPoint(150.0u"pixel", 275.0u"pixel")
        
        # Test fractional pixels
        pp_sub = ProjectionPoint(100.5u"pixel", 200.25u"pixel")
        @test pp_sub.x == 100.5u"pixel"
        @test pp_sub.y == 200.25u"pixel"
    end
    ...
    @testset "Camera Projection" begin
        ...
        @test proj_near.x ≈ proj_far.x atol=1e-6u"pixel"
        @test proj_near.y ≈ proj_far.y atol=1e-6u"pixel"
    end
end
